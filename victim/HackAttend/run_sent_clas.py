...
from modelling.bert import BertForSequenceClassification
from modelling.deberta import DebertaV2ForSequenceClassification
...
class HansProcessor(MultiFormatDataProcessor):
    """Processor for the HANS data set."""

    def get_train_examples(self, data_dir):
        """See base class."""
        return self.get_examples_from_file(
            os.path.join(data_dir, "heuristics_train_set.jsonl"),
            None,
            "train",
        )
    ...
    def get_dev_examples(self, data_dir):
        """See base class."""
        return NotImplementedError("HANS does not have a dev set")

    def get_test_examples(self, data_dir):
        """See base class."""
        return self.get_examples_from_file(
            os.path.join(data_dir, "heuristics_evaluation_set.jsonl"),
            None,
            "test",
        )
    ...
    def _create_examples(self, records, labels=None, set_type="predict"):
        ...
        for i, (record, label) in enumerate(zip(records, labels)):
            ...
            examples.append(
                InputExample(guid=guid, text_a=text_a, text_b=text_b, label=gold_label)
            )
        return examples
...
class MnliProcessor(DataProcessor):
    ...
    def get_adv_examples(self, data_dir):
        return self._create_examples(
            self._read_tsv(os.path.join(data_dir, "adv", "dev_matched.tsv")),
            "dev_matched_adv",
        )
...
def main():
    ...
    parser.add_argument(
        "--do_test", action="store_true", help="Whether to evaluate on the test set."
    )
    parser.add_argument(
        "--do_lower_case",
        action="store_true",
    ...
    parser.add_argument(
        "--weight_decay", type=float, default=0.01, help="L2 weight decay for training."
    )
    parser.add_argument(
    ...
    tokenizer = AutoTokenizer.from_pretrained(
        args.load_model_path, do_lower_case=args.do_lower_case, cache_dir=cache_dir
    )
    ...
    if args.do_train:
        ...
        if args.do_eval:
            ...
            #     os.path.join(args.data_dir, task_name)
            # )
            # renamed dev set to test set with download data script
        ...
        for epoch in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            if args.do_eval:
                ...
                predict_model = AutoModel[args.model_type].from_pretrained(
                    args.load_model_path,
                    num_labels=num_labels,
                    return_dict=True,
                    cache_dir=cache_dir,
                ...
                for batch in tqdm(eval_dataloader, desc="Evaluation"):
                    ...
                    if task_name == "sts-b":
                        ...
                        all_predictions.extend(
                            np.argmax(logits, axis=1).squeeze().tolist()
                        )
                        all_labels.extend(label_ids.squeeze().tolist())
                ...
                if result["eval_acc"] > best_result:
                    ...
                    output_model_file = os.path.join(
                        args.output_dir, "best_pytorch_model.bin"
                    )
                    torch.save(model_to_save.state_dict(), output_model_file)
            ...
            logger.info(
                "Best epoch: %s, result:  %s", str(best_epoch), str(best_result)
            )
    ...
    if args.do_test:
        ...
        if task_name != "mnli":
            ...
            output_test_file = os.path.join(
                args.output_dir, "{}.tsv".format(args.task_name)
            )
        else:
            output_test_file = os.path.join(
                args.output_dir, "{}-m.tsv".format(args.task_name)
            )
        ...
        if task_name == "mnli":
            eval_examples = processor.get_test_examples(
                os.path.join(args.data_dir, task_name), "mm"
            )
            eval_features = convert_examples_to_features(
                eval_examples, label_list, args.max_seq_length, tokenizer
            )
            ...
            output_test_file = os.path.join(
                args.output_dir, "{}-mm.tsv".format(args.task_name)
            )
            with open(output_test_file, "w") as writer:
                writer.write("index" + "\t" + "prediction" + "\n")
                for index, pred in enumerate(predictions):
